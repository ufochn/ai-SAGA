...
class MainFlutterWindow: NSWindow {
  override func awakeFromNib() {
    ...
    registerHardwareKeyChannel(flutterViewController)

    super.awakeFromNib()
  }

  // MARK: - 安全硬件密钥通道（Keychain / Secure Enclave）

  /// 注册与 Android / iOS 端一致的通道：ai_saga/hardware_key
  ///  - getPublicKey：从 Keychain（优先 Secure Enclave）获取/生成 ECDSA P-256 公钥（Base64，DER SubjectPublicKeyInfo）
  ///  - sign：用 Keychain 私钥对 Base64 数据做 ECDSA SHA256 签名（X9.62 DER）
  ///  - hasKey：查询本机是否已有密钥
  private func registerHardwareKeyChannel(_ controller: FlutterViewController) {
    ...
    channel.setMethodCallHandler { [weak self] call, result in
      guard let self else {
        result(FlutterError(code: "NO_APP", message: "MainFlutterWindow 不可用", details: nil))
        return
      }
      switch call.method {
      case "getPublicKey":
  ...
  private static let keyTag = "com.example.ai_saga.devicekey.macos".data(using: .utf8)!

  /// 查询不限定 Token ID，Secure Enclave 与普通 Keychain 密钥均可命中
  ...
  private func getOrCreatePublicKey() throws -> String {
    ...
    guard let pubKey = try? loadPublicKey() else {
      throw NSError(domain: "hardware_key", code: -1, userInfo: [NSLocalizedDescriptionKey: "无法读取 Keychain 公钥"])
    }
    let derData = try publicKeyDER(pubKey)
    return derData.base64EncodedString()
  }

  /// 生成 ECDSA P-256 密钥对：优先 Secure Enclave，不可用时降级到普通 Keychain。
  private func generateKeyPair(useSecureEnclave: Bool = true) throws {
    ...
    var attributes: [String: Any] = [
      ...
      kSecAttrLabel as String: Self.keyAlias,
      kSecAttrApplicationTag as String: Self.keyTag,
    ...
    guard SecKeyCreateRandomKey(attributes as CFDictionary, &error) != nil else {
      if useSecureEnclave {
        // Secure Enclave 不可用（非 Apple Silicon / 无 T2、虚拟机、调试未签名等）→ 降级
        try generateKeyPair(useSecureEnclave: false)
        return
      }
      let message = error?.takeRetainedValue().localizedDescription ?? "生成 Keychain 密钥失败"
  ...
  private func loadPublicKey() throws -> SecKey {
    ...
    let status = SecItemCopyMatching(keyQuery as CFDictionary, &rawItem)
    guard status == errSecSuccess,
          let ref = rawItem,
          CFGetTypeID(ref) == SecKeyGetTypeID() else {
      throw NSError(domain: "hardware_key", code: -3, userInfo: [NSLocalizedDescriptionKey: "未找到 Keychain 公钥"])
    }
    let key = ref as! SecKey
    // 取回的可能私钥引用，显式取出对应公钥，确保导出的总是 SubjectPublicKeyInfo
    guard let publicKey = SecKeyCopyPublicKey(key) else {
      throw NSError(domain: "hardware_key", code: -8, userInfo: [NSLocalizedDescriptionKey: "无法获取 Keychain 公钥"])
    }
    return publicKey
  }
  ...
  private func publicKeyDER(_ pubKey: SecKey) throws -> Data {
    ...
    // EC 公钥的 external representation 是 X9.63 格式：0x04 || X(32) || Y(32)，
    // 需要包装成 SubjectPublicKeyInfo 以便与 Android / 服务端互操作。
  ...
  /// 用 Keychain 私钥对 Base64 数据做 ECDSA SHA256 签名，返回 Base64（DER X9.62）。
  private func sign(dataB64: String) throws -> String {
    ...
    guard status == errSecSuccess,
          let ref = rawItem,
          CFGetTypeID(ref) == SecKeyGetTypeID() else {
      throw NSError(domain: "hardware_key", code: -6, userInfo: [NSLocalizedDescriptionKey: "无法读取 Keychain 私钥"])
    }
    ...
    guard let sig = SecKeyCreateSignature(
      ...
      let message = error?.takeRetainedValue().localizedDescription ?? "Keychain 签名失败"
